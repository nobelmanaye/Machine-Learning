...
age = list(df['age'])
pnemonage = [(age[i])*(int(math.e))**(3*pnemon[i]) for i in range(len(age))]
...
cols = ['pneumonia','age','agepnemonia','icu','covid_res','asthma']
hs = df[cols]
#minmax(cols,df)


minm =minmax(cols,df[cols])
scaled = minmax(cols,df[cols])
...
rob = robust(cols,df[cols])


sta = stdscaler(cols,df[cols])
abs = maxAbs(cols,df[cols])

print("MINIM " + str(minm.std()) +"B")

print("rob" + str(rob.std()))

print("sta" + str(sta.std()))

print("abs" + str(abs.std()))
